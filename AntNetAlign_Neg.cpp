/***************************************************************************
                           AntNetAlign_Neg.cpp  -  description
                             -------------------
...
double negative_learning_rate = 0.1;
...
double t_sub = 60; //Subinstance max time in seconds (Default: 1min)
int emphasis = 1; //MIP Emphasis 0-4
...
struct cmp {
    bool operator() (PossAlign a, PossAlign b) const {
...
void read_parameters(int argc, char **argv) {
	...
	while (iarg < argc) {
		...
		else if (strcmp(argv[iarg],"-neg_l_rate")==0) negative_learning_rate = atof(argv[++iarg]);
		...
		else if (strcmp(argv[iarg],"-t_sub")==0) t_sub = atoi(argv[++iarg]);
		else if (strcmp(argv[iarg],"-tuning")==0) tuning = true;
		else if (strcmp(argv[iarg],"-emphasis")==0) emphasis = atoi(argv[++iarg]);
...
bool solve_subinstance(Solution& sol, const vector<set<PossAlign,cmp>>& poss_align_g1, const vector<set<PossAlign,cmp>>& poss_align_g2, int n_possible_alignments, Solution& bestCurrentSol){
	
	if(!tuning) cout << endl << "-> Generating subinstance..." << endl;
	
	double start_ILP_generator = getCurrentTime();
	...
	//CPLEX model. Variables + constraints.
	IloModel model(env);

	//--- VARIABLES ---
	IloNumVarArray align(env,n_possible_alignments);
	IloNumVarArray conservative(env,0);

	//--- CONSTRAINTS ---
	IloRangeArray exactly_one_g1(env, V1);  
	IloRangeArray at_most_one_g2(env, V2);
	IloRangeArray conservative_constraint(env,0);

	//--- Create variables ---
	for(int i = 0; i < n_possible_alignments; ++i) align[i] = IloNumVar(env,0,1,ILOBOOL);

	// --- Create constraints ---
	IloExpr expr(env);
	
	//--- Warm start ---
	IloNumVarArray xVariables(env);
	IloNumArray xValues(env);

	//Each node from G1 is aligned to exactly one from G2 O(V1)
	for(int i = 0; i < V1; ++i){
		for(set<PossAlign>::iterator it = poss_align_g1[i].begin(); it != poss_align_g1[i].end(); ++it){
			expr += align[(*it).id];
			//Warm start
			xVariables.add(align[(*it).id]);
			if((*it).vertex != bestCurrentSol.alignment[i]) xValues.add((int)0);
			else xValues.add(1);
		}
		exactly_one_g1[i] = IloRange(env,1,expr,1);
		expr.clear();
		
	}
	model.add(exactly_one_g1);

	//Each node from G2 can be aligned to at most one node from G1 O(V2)
	for(int j = 0; j < V2; ++j){
		for(set<PossAlign>::iterator it = poss_align_g2[j].begin(); it != poss_align_g2[j].end(); ++it){
			expr += align[(*it).id];
		}
		at_most_one_g2[j] = IloRange(env,0,expr,1);
		expr.clear();
		
	}
	model.add(at_most_one_g2);
	
	//A pair of alignments is conservative if they can conserve an edge (both pair of nodes are connected)
	int n_pair_alignments = 0;
	int currentNumberOfConservedEdges = 0;
	//Iterate over the edges (of G1)
	for(int u1 = 0; u1 < V1; ++u1){
		for(set<int>::iterator it = G1[u1].begin(); it != G1[u1].end(); ++it){
			int v1 = *it;
			//Count edges only once
			if(u1 <= v1){
				//Iterate over possible pairs of nodes that the endpoints are mapped to
				for(set<PossAlign>::iterator p1 = poss_align_g1[u1].begin(); p1 != poss_align_g1[u1].end(); ++p1){
					for(set<PossAlign>::iterator p2 = poss_align_g1[v1].begin(); p2 != poss_align_g1[v1].end(); ++p2){
						int u2 = (*p1).vertex;
						int v2 = (*p2).vertex;
						//Target nodes are connected (conservative pair of alignments)
						if(AdjMat_G2[v2][u2]){
							conservative.add(IloNumVar(env,0,1,ILOBOOL));
							expr -= 2*conservative[n_pair_alignments];
							expr += align[(*p1).id];
							expr += align[(*p2).id];
							conservative_constraint.add(IloRange(env,0,expr,1));
							expr.clear();
							//Warm start
							xVariables.add(conservative[n_pair_alignments]);
							if(bestCurrentSol.alignment[u1] == u2 and bestCurrentSol.alignment[v1] == v2) {
								xValues.add(1);
								currentNumberOfConservedEdges++;
							}
							else xValues.add((int)0);
							n_pair_alignments++;
						}
					}
				}
	...
	model.add(conservative_constraint);

	//Objective function (Maximize EC)
	for(int i = 0; i < n_pair_alignments; ++i)
		expr += conservative[i];
	...
	//Warm start
	cplex.addMIPStart(xVariables,xValues);
	xVariables.end();
	xValues.end();

	//Set time limit
	cplex.setParam(IloCplex::TiLim, t_sub*(1-cf));
	
	//One thread mode (Sequential)
	cplex.setParam(IloCplex::Param::Threads, 1);
	
	//Emphasis
	cplex.setParam(IloCplex::Param::Emphasis::MIP,emphasis);
	
	double elapsed_ILP_generator = getCurrentTime() - start_ILP_generator;
	if(!tuning) cout << "Time creation ILP model: " << elapsed_ILP_generator << endl;

	if(!tuning) cout << "-> Solving with CPLEX..." << endl;
	bool solved = false;
	try {
	...
	if(solved) {
		...
		if(!tuning) std::cout << "CPLEX success!\n";

		IloNumArray obtained_alignment(env);
		cplex.getValues(obtained_alignment, align);
	
		//Read solution
		for(int i = 0; i < V1; ++i){
			for(set<PossAlign>::iterator it = poss_align_g1[i].begin(); it != poss_align_g1[i].end(); ++it){
				int j = (*it).vertex;
				//Vertex i from G1 aligned to vertex j from G2
				if(obtained_alignment[(*it).id] > 0){
					sol.alignment[i] = j;
				}
			}	
		}
		//Calculate Scores
		int num_induced_edges_g2 = calculate_induced_G2(sol.alignment);
		int num_conserved_edges = (int)cplex.getObjValue();
		sol.scores["EC"] = (double)num_conserved_edges/(double)E1;
		sol.scores["ICS"] = (double)num_conserved_edges/(double)num_induced_edges_g2;
		sol.scores["S3"] = (double)num_conserved_edges/(double)(num_induced_edges_g2+E1-num_conserved_edges);
...
//Generates a solution given a positive and negative pheromone matrix
void generate_aco_solution(Solution& sol, vector<vector<double>>& positive_pheromones, vector<vector<double>>& negative_pheromones) {
	...
	while (num_unaligned_vertices > 0) {
		...
		for (set<int>::iterator cit = unaligned_g2.begin(); cit != unaligned_g2.end(); ++cit) {
			...
			Option opt;
			opt.vertex = *cit;
			opt.value = h_func*positive_pheromones[current_vertex][*cit]*(1-negative_pheromones[current_vertex][*cit]);
...
int main( int argc, char **argv ) {
	...
	vector<vector<double>> negative_pheromones(V1,vector<double>(V2,tau_min));
	...
	while (n_constructions < max_constructions && (getCurrentTime() - startingTime) < max_running_time) {
		...
		//Store possible alignments
		vector<set<PossAlign,cmp>> poss_align_g1(V1,set<PossAlign,cmp>());
		vector<set<PossAlign,cmp>> poss_align_g2(V2,set<PossAlign,cmp>());
		int n_possible_alignments = 0;
		pair<set<PossAlign,cmp>::iterator,bool> ret;
		...
		for (int na = 0; na < n_of_ants; ++na) {
			...
			generate_aco_solution(iSol, positive_pheromones,negative_pheromones);
			...
			for(int i = 0; i < V1; ++i) {
				PossAlign pos;
				pos.vertex = iSol.alignment[i];
				pos.id = n_possible_alignments;
				ret = poss_align_g1[i].insert(pos);
				//New alignment
				if(ret.second){
					pos.vertex = i;
					poss_align_g2[iSol.alignment[i]].insert(pos);
					n_possible_alignments++;
				}
			}
		}
		...
		if(solve_subinstance(subSol,poss_align_g1,poss_align_g2,n_possible_alignments,iBestSol)){
			if(!tuning) cout << endl << "[ILP]\tS3: " << subSol.scores["S3"] << "\tEC: " << subSol.scores["EC"] << "\tICS: " << subSol.scores["ICS"] << endl;
			if (subSol.scores[score] > iBestSol.scores[score]) iBestSol = subSol;
			if (subSol.scores[score] > restartSol.scores[score]) restartSol = subSol;
			if (subSol.scores[score] > bestSol.scores[score]) {
				bestSol = subSol;
				result = bestSol.scores[score];
				time = getCurrentTime() - startingTime;
				bestSol.time = time;
		...
		//Negative pheromone update using the contributions and taking into account the negative learning rate
		//Count only those alignments that contribute to the solution (those that conserve edges)
		for(int u1 = 0; u1 < V1; ++u1){
			for(set<PossAlign,cmp>::iterator it1 = poss_align_g1[u1].begin(); it1 != poss_align_g1[u1].end(); ++it1){
				int u2 = (*it1).vertex;
				//This component does not form part of the best solution
				if(bestSol.alignment[u1] != u2) negative_pheromones[u1][u2] += (negative_learning_rate * (1 - negative_pheromones[u1][u2]));
				else {
					//See if u1 is an endpoint of a conserved edge
					bool exists_conserved = false;
					for(set<int>::iterator it2 = G1[u1].begin(); it2 != G1[u1].end() and not exists_conserved; ++it2){
						int v1 = *it2;
						int v2 = bestSol.alignment[v1];
						if(AdjMat_G2[u2][v2]) exists_conserved = true;
					}
					//This component is part of the best solution and conserves and edge
					if(exists_conserved) negative_pheromones[u1][u2] += (negative_learning_rate * (0 - negative_pheromones[u1][u2]));
					//This component is part of the best solution but does not conserve and edge
					else negative_pheromones[u1][u2] += (negative_learning_rate * (1 - negative_pheromones[u1][u2]));
				}
				if (negative_pheromones[u1][u2] > tau_max) negative_pheromones[u1][u2] = tau_max;
				if (negative_pheromones[u1][u2] < tau_min) negative_pheromones[u1][u2] = tau_min;
			}
		}
		...
		if (cf > 0.9999) {
			...
			if (global_convergence) {
				...
				negative_pheromones = vector<vector<double>>(V1,vector<double>(V2,tau_min));
				restartSol.scores["EC"] = std::numeric_limits<double>::lowest();
				restartSol.scores["ICS"] = std::numeric_limits<double>::lowest();
				restartSol.scores["S3"] = std::numeric_limits<double>::lowest();
			}
